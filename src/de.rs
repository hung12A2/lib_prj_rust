...
//! 
//! Deserializer là một thành phần trong lập trình, 
//! có nhiệm vụ chuyển đổi dữ liệu được lưu trữ trong một dạng nhất định 
//! (thường là chuỗi hoặc mảng byte) thành một đối tượng hoặc cấu trúc dữ liệu khác 
//! (thường là đối tượng hoặc struct trong ngôn ngữ lập trình). 
//! Deserializer được sử dụng rộng rãi trong việc lưu trữ 
//! và truyền dữ liệu giữa các hệ thống và giữa các ngôn ngữ lập trình.
...
/// Kiểu dữ liệu phục vụ cho việc giải mã JSON -> rust
...
where
    R: read::Read<'de>,
{
    /// JSON deserialize từ những đầu vào khả dụng
    ///
    /// Thông thường, sử dụng một trong các phương thức sau sẽ rất tiện dụng hơn
    ///
    ...
    R: crate::io::Read,
{
    ...
    ///
    /// Deserialized không hỗ trợ giải mã các kiểu được cho mượn 
    /// Như là &str 
    /// Tất cả các hoạt động đều dựa trên việc sao chép dữ liệu
    /// Do IoRead không có phương thức yêu cầu không sao chép 
...
impl<'a> Deserializer<read::SliceRead<'a>> {
    /// Tạo ra 1 function giải mã JSON từ bytes 
...
impl<'a> Deserializer<read::StrRead<'a>> {
    /// Tạo ra 1 function giải mã JSON từ &str 
...
impl<'de, R: Read<'de>> Deserializer<R> {
    /// Phương thức Deserializer::end nên được gọi 
    /// sau khi một giá trị đã được deserialize đầy đủ. 
    /// Điều này cho phép Deserializer kiểm tra xem luồng đầu vào 
    /// đã đến cuối hoặc chỉ còn khoảng trắng cuối.
    ...
    ///Chuyển đổi một Deserializer JSON thành một tiến trình lặp qua các giá trị của kiểu T.

    pub fn into_iter<T>(self) -> StreamDeserializer<'de, R, T>
    where
        T: de::Deserialize<'de>,
    {
        // Cần lựa chọn T là gì ? 
    ...
    /// Phân tích cấu trúc Json bất kì -> dữ liệu trong rust
    /// Mà không gặp vấn đề về bộ nhớ 
    ///

    /// *This method is only available if serde_json is built with the
    /// `"unbounded_depth"` feature.*
    /// Phương thức này tồn tại, nếu serde_json được build với 
    /// "unbounded_depth" feature 
    ///
    /// # Examples
    ///
    ...
    /// Error gây ra bơi 1 byte từ next_char()
    #[cold]
    ...
    /// Error gây ra bởi 1 byte từ peek ()
    #[cold]
    ...
    /// Trả về byte đầu tiên không phải khoảng trắng 
                ...
                loop {
                    match tri!(self.peek_or_null()) {
                        c @ b'0'..=b'9' => {
                            ...
                            // Chúng ta cần cẩn thận với việc tràn số. Nếu có thể
                            // Ta nên cố gắng để chuyển số thành dạng u64 cho tới khi 
                            // số đó trở nên quá lớn 
                            // Với số có phẩy, nên chuyển thành dạng f64
    ...
    fn parse_long_integer(&mut self, positive: bool, partial_significand: u64) -> Result<f64> {
        /// Để giải mã hoá số thực, 
        /// chúng ta sẽ trước tiên đẩy phần nguyên và phần số thập phân, 
        /// cả hai là chuỗi byte, vào bộ đệm và 
        /// sau đó cung cấp cả hai phần để giải mã bởi parser của lexical. 
        /// Ví dụ, nếu đầu vào là 12.34e5, 
        /// chúng ta sẽ đẩy b"1234" vào bộ đệm và sau đó truyền b"12" và b"34" cho lexical. 
        /// integer_end sẽ được sử dụng để theo dõi nơi chia bộ đệm.

        ///Lưu ý rằng lexical mong muốn phần nguyên chứa không có số 0 ở đầu 
        /// và phần số thập phân không chứa không có số 0 ở cuối. 
        /// Yêu cầu đầu tiên đã được xử lý bởi các quy tắc giải mã số nguyên. 
        /// Yêu cầu thứ hai sẽ được thực hiện trước khi truyền các phần 
        /// để giải mã cho lexical trong f64_long_from_parts.
        self.scratch.clear();
        self.scratch
    ...
    fn parse_long_integer(&mut self, positive: bool, significand: u64) -> Result<f64> {
        ...
        loop {
            match tri!(self.peek_or_null()) {
                b'0'..=b'9' => {
                    self.eat_char();
                    // Có thể gây ra tràn số 
                    // Tuy nhiên ta bỏ qua trường hợp này 
                    exponent += 1;
                }
                b'.' => {
    ...
    fn parse_decimal_overflow(
        ...
        // Lần nhân tiếp theo sẽ gây tràn số
        // 
...
impl<'de, 'a, R: Read<'de>> de::Deserializer<'de> for &'a mut Deserializer<R> {
    ...
    /// Phân tích 1 JSON string -> bytes. Chức năng này không kiểm tra
    /// liệu các byte có đại diện cho 1 chuỗi hợp lệ hay không 
    /// ```
    ...
    /// Phân tích null như là None và bất kỳ giá trị khác như Some(...).
    #[inline]
    ...
    /// Phân tích một enum như một đối tượng như {"$KEY":$VALUE}, 
    /// trong đó $VALUE là một giá trị trực tiếp
    #[inline]
...
/// Chỉ deserialize từ điều này sau khi dò được một byte '"'! Nếu không, 
/// nó có thể deserialize được JSON không hợp lệ thành công.
struct MapKey<'a, R: 'a> {
    de: &'a mut Deserializer<R>,
}
...
/// Lần lượt giải mã nhiều giá trị JSON từ một luồng.
///
/// A stream deserializer có thể được tạo ra từ bất kỳ JSON deserializer
/// sử dụng phương thức Deserializer::into_iter.
///
/// Dữ liệu có thể bao gồm bất kỳ giá trị JSON. 
/// Các giá trị cần phải là một giá trị tự chỉ định, 
/// ví dụ mảng, đối tượng hoặc chuỗi hoặc được theo sau bởi 
/// khoảng trắng hoặc một giá trị tự chỉ định.
///
/// ```
...
where
    ...
    T: de::Deserialize<'de>,
{
    /// Tạo trình giải tuần tự hóa luồng JSON 
    /// từ một trong các nguồn đầu vào có thể có của serde_json 
    ///
    /// Thông thường sẽ đơn giản hơn nếu sử dụng
    ///
...
/// # Errors
///
/// Chuyển đổi này có thể thất bại nếu cấu trúc của đầu vào không khớp với cấu trúc mong đợi của T
/// ví dụ nếu T là struct type nhưng đầu vào chứa một cái gì đó khác hơn 1 map JSON 
/// Nó cũng có thể thất bại nếu cấu trúc là đúng nhưng T thực hiện của Deserialize 
/// nhận định rằng có gì đó sai với dữ liệu, 
/// ví dụ các trường cần thiết của struct bị thiếu trong map JSON 
/// hoặc một số số quá lớn để phù hợp với kiểu số nguyên dự kiến.
